...
def check_steam_sync(username, password):
    ...
    try:
        ...
        if not rsa_data or not rsa_data.get("success"):
            return {"status": "error", "response": "Failed to get RSA key"}
        ...
        if not res:
             return {"status": "error", "response": "No response from Steam login"}
             
        if not res.get("success"):
             msg = res.get("message", "Login Failed")
             if res.get("captcha_needed"):
                 msg = "Captcha Required (Failed)"
             if res.get("emailauth_needed"):
                 msg = "Steam Guard Required (Use Code)"
             return {"status": "failed", "response": msg}
        ...
        capture = {
            ...
            "GamesList": games_list[:5], # First 5 games
            "VAC": vac_list,
            "GameBans": gameban_list,
            "SteamGuard": "Enabled" if client.cookies.get("steamLoginSecure") else "Unknown"
        }
        return {"status": "success", "response": "Login Successful", "capture": capture, "steamid": steamid}
        
    except Exception as e:
        return {"status": "error", "response": str(e)}

async def check_steam_account(username, password):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, check_steam_sync, username, password)
